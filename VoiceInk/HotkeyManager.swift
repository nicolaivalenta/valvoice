...
class HotkeyManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.nicolaivalenta.valvoice", category: "HotkeyManager")
    ...
    private var lastCapsLockToggleTime: TimeInterval = 0
    ...
    init(engine: ValVoiceEngine, recorderUIManager: RecorderUIManager) {
        self.selectedHotkey1 = HotkeyOption(rawValue: UserDefaults.standard.string(forKey: "selectedHotkey1") ?? "") ?? .capsLock
    ...
    private func setupModifierKeyMonitoring() {
        ...
        // If caps lock is selected, set up a CGEvent tap to intercept and suppress it
        if selectedHotkey1 == .capsLock || selectedHotkey2 == .capsLock {
            setupCapsLockEventTap()
        }
    ...
    private func setupCapsLockEventTap() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                // Caps Lock keycode is 0x39 (57)
                if keycode == 0x39 {
                    // Trigger the toggle on the main actor
                    if let refcon = refcon {
                        let manager = Unmanaged<HotkeyManager>.fromOpaque(refcon).takeUnretainedValue()
                        Task { @MainActor in
                            await manager.handleCapsLockPress()
                        }
                    }
                    // Suppress the event so caps lock typing state doesn't change
                    return nil
                }
            ...
            logger.warning("Failed to create caps lock event tap — Accessibility permission may be needed")
            return
        }

        capsLockEventTap = tap
        capsLockRunLoopSource = CFMachPortCreateRunLoopSource(kCFAllocatorDefault, tap, 0)
        if let source = capsLockRunLoopSource {
            CFRunLoopAddSource(CFRunLoopGetMain(), source, .commonModes)
        }
        CGEvent.tapEnable(tap: tap, enable: true)
        logger.notice("Caps lock event tap installed")
    }

    private func handleCapsLockPress() async {
        // Debounce: caps lock can fire multiple events per physical press
        let now = ProcessInfo.processInfo.systemUptime
        guard now - lastCapsLockToggleTime > 0.3 else { return }
        lastCapsLockToggleTime = now

        guard canProcessHotkeyAction else { return }
        logger.notice("handleCapsLockPress: toggling mini recorder")
    ...
    private func handleModifierKeyEvent(_ event: NSEvent) async {
        ...
        switch hotkey {
        case .capsLock:
            // Caps lock is handled by the CGEvent tap — skip NSEvent processing
            return
    ...
    static func setCapsLockLED(_ on: Bool) {

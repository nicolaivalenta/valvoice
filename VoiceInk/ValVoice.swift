...
@main
struct ValVoiceApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) var appDelegate
    let container: ModelContainer
    let containerInitializationFailed: Bool

    @StateObject private var engine: ValVoiceEngine
    ...
    init() {
        ...
        let logger = Logger(subsystem: "com.nicolaivalenta.valvoice", category: "Initialization")
        let schema = Schema([
            Transcription.self,
            VocabularyWord.self,
        ...
        // Attempt 2: Try in-memory storage
            ...
            DispatchQueue.main.async {
                ...
                alert.informativeText = "ValVoice couldn't access its storage location. Your transcriptions will not be saved between sessions."
    ...
    private static func createPersistentContainer(schema: Schema, logger: Logger) -> ModelContainer? {
        do {
            ...
            // Dictionary configuration — ValVoice is local-only, no CloudKit sync
    ...
    var body: some Scene {
        WindowGroup {
            if hasCompletedOnboarding {
                ContentView()
                    ...
                    .onAppear {
                        ...
                        if containerInitializationFailed {
                            ...
                            alert.informativeText = "ValVoice cannot initialize its storage system. The app cannot continue.\n\nPlease try reinstalling the app or contact support if the issue persists."
                ...
                OnboardingView(hasCompletedOnboarding: $hasCompletedOnboarding)
                    ...
                    .background(WindowAccessor { window in
                        if window.identifier == nil || window.identifier != NSUserInterfaceItemIdentifier("com.nicolaivalenta.valvoice.onboardingWindow") {
        ...
        // ValVoice: menu bar icon removed entirely — app is controlled by hotkey.

...
struct OnboardingPermissionsView: View {
    ...
    private let permissions: [OnboardingPermission] = [
        ...
        OnboardingPermission(
            ...
            description: "Select the audio input device you want to use with ValVoice.",
            icon: "headphones",
            type: .audioDeviceSelection
        ),
        OnboardingPermission(
            title: "Accessibility Access",
            description: "Allow ValVoice to help you type anywhere in your Mac.",
            icon: "accessibility",
            type: .accessibility
        ),
        OnboardingPermission(
        ...
        OnboardingPermission(
            ...
            description: "Set up a keyboard shortcut to quickly access ValVoice from anywhere.",
            icon: "keyboard",
                            ...
                            VStack(spacing: 12) {
                                HStack(spacing: 8) {
                                    ...
                                    if permissions[currentPermissionIndex].type == .screenRecording {
                                        InfoTip(
                                            "ValVoice captures on-screen text to understand the context of your voice input, which significantly improves transcription accuracy. Your privacy is important: this data is processed locally and is not stored.",
    ...
    private func checkExistingPermissions() {
        ...
        // Auto-advance to first ungranted permission so app-restart resumes where user left off
        if let firstUngranted = permissionStates.firstIndex(where: { !$0 }) {
            currentPermissionIndex = firstUngranted
        } else {
            // All granted — skip straight to model download
